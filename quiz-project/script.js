...
function nextqns() {
  userAnswers.push(selectedAnswer !== null ? selectedAnswer : null);
...
function showResult() {
  ...
  Questions.forEach((qns, index) => {
    ...
    qnsShow.innerText = `${index + 1}.${Questions[index].question}`;
    ...
    correctAnswer.classList.add("text-success", "mt-4");

    let selectedAnswer = document.createElement("p");
    selectedAnswer.innerText = `your Ans : ${userAns}`;
    if (userAnswers[index] === qns.correctIndex) {
